...
def monitor_cpu_usage():
    """Get CPU usage and core count with reduced frequency."""
    try:
...
def process_uploaded_files(uploaded_files):
    """Process uploaded files and run backend logic, showing time and CPU graph at the end."""
...
def display_max_plagiarism_per_file(df):
    """Display each file's highest plagiarism match."""
    start_time = time.perf_counter()
    ...
    if "selected_file" in st.session_state:
        ...
        with st.expander(f"📄 Content of {selected_file}", expanded=True):
...
def display_filtered_results(df):
    """Display filtered plagiarism results with highlighting."""
    start_time = time.perf_counter()
